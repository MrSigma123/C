...
Program wczytuje bazę zdań z pliku,
każde zdanie w osobnej linii.
Następnie wyświetla użytkownikowi jedno zdanie
i każe je przepisać.
Użytkownik wpisuje tekst,
jednak nie widzi wpisanego tekstu na ekranie.
Po naciśnięciu "enter" program wyświetla
wpisane przez użytkownika zdanie,
wyraźnie pokazuje błędy oraz pokazuje
procentową ilość błędów
(za dobre uznajemy właściwe litery
na właściwych pozycjach).
Do tego program pokazuje czas,
jaki upłynął od naciśnięcia
